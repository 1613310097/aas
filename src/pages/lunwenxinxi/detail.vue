	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/lunwenxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.lunwenmingcheng}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">检查项</div>
						<div class="text "  >{{detail.jianchaxiang}}</div>
					</div>
					<div class="item">
						<div class="lable">检查对象</div>
						<div class="text "  >{{detail.jianchaduixiang}}</div>
					</div>
					<div class="item">
						<div class="lable">完成状态</div>
						<div class="text "  >{{detail.wanchengzhuangtai}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<el-button class="uploadBtn" @click="download(detail.wenjian )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('lunwenxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('lunwenxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('lunwenxinxi','检查操作')" 
							@click="onAcross('jianchajilu','','','wanchengzhuangtai','已审核','已审核,未审核'.split(',')[0])" type="warning">
							检查操作
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="任务内容" name="first">
					<div class="ql-snow ql-editor" v-html="detail.renwuneirong"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`lunwenxinxi`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/lunwenxinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/lunwenxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此论文信息？') .then(_ => {
					this.$http.post('lunwenxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
