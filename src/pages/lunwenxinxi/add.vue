<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="论文名称" prop="lunwenmingcheng">
				<el-input v-model="ruleForm.lunwenmingcheng" 
					placeholder="论文名称" clearable :disabled=" false  ||ro.lunwenmingcheng"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item"  label="检查项" prop="jianchaxiang">
				<el-select v-model="ruleForm.jianchaxiang" placeholder="请选择检查项" :disabled=" false  ||ro.jianchaxiang" multiple filterable>
					<el-option
			...
			<el-form-item class="add-item"  label="检查对象" prop="jianchaduixiang">
				<el-select v-model="ruleForm.jianchaduixiang" placeholder="请选择检查对象" :disabled=" false  ||ro.jianchaduixiang" >
					<el-option
			...
			<el-form-item class="add-item"  label="完成状态" prop="wanchengzhuangtai">
				<el-select v-model="ruleForm.wanchengzhuangtai" placeholder="请选择完成状态" :disabled="true  ||ro.wanchengzhuangtai" >
					<el-option
			...
			<el-form-item class="add-item" label="任务内容" prop="renwuneirong">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='lunwenmingcheng'){
							this.ruleForm.lunwenmingcheng = obj[o];
							this.ro.lunwenmingcheng = true;
							continue;
						}
				...
				this.$http.get('option/jianchaxiang/jianchaxiang', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
				...
				this.$http.get('option/jiaoshi/jiaoshigonghao', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.jianchaduixiangOptions = res.data.data;
					}
				});
			...
			jiaoshigonghaoChange () {
				this.$http.get('follow/jiaoshi/jiaoshigonghao?columnValue=' + this.ruleForm.jiaoshigonghao, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if(res.data.data.lianxishouji){
							this.ruleForm.lianxishouji = res.data.data.lianxishouji
						}
			...
			// 多级联动参数
			info() {
				this.$http.get(`lunwenxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`lunwenxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
