	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jianchajilu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.wanchengshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">完成报告</div>
						<el-button class="uploadBtn" @click="download(detail.wanchengbaogao )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jianchajilu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jianchajilu','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jianchajilu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jianchajiluAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此检查记录？') .then(_ => {
					this.$http.post('jianchajilu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
