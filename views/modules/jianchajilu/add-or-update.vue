	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="检查项" prop="jianchaxiang" >
					<el-input v-model="ruleForm.jianchaxiang" placeholder="检查项" clearable  :readonly="ro.jianchaxiang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="检查项" prop="jianchaxiang" >
					<el-input v-model="ruleForm.jianchaxiang" placeholder="检查项" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="检查对象" prop="jianchaduixiang" >
					<el-input v-model="ruleForm.jianchaduixiang" placeholder="检查对象" clearable  :readonly="ro.jianchaduixiang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="检查对象" prop="jianchaduixiang" >
					<el-input v-model="ruleForm.jianchaduixiang" placeholder="检查对象" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="upload" v-if="type!='info'&& !ro.wanchengbaogao" label="完成报告" prop="wanchengbaogao" >
					<file-upload
						tip="点击上传完成报告"
						action="file/upload"
						:limit="1"
				...
				<el-form-item v-else-if="ruleForm.wanchengbaogao" label="完成报告" prop="wanchengbaogao" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.wanchengbaogao)">
	...
	export default {
		...
		methods: {
			...
			async onSubmit() {
					...
					if(this.ruleForm.wanchengbaogao!=null) {
						this.ruleForm.wanchengbaogao = this.ruleForm.wanchengbaogao.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jianchajilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jianchajiluCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			wanchengbaogaoUploadChange(fileUrls) {
				this.ruleForm.wanchengbaogao = fileUrls;
			},
		}
	};
</script>
