		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="论文名称" prop="lunwenmingcheng" >
					<el-input v-model="ruleForm.lunwenmingcheng" placeholder="论文名称" clearable  :readonly="ro.lunwenmingcheng"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="论文名称" prop="lunwenmingcheng" >
					<el-input v-model="ruleForm.lunwenmingcheng" placeholder="论文名称" readonly></el-input>
				</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `lunwenxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.lunwenxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
